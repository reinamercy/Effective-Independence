...
% Coverage reached 360/360 on 2026-09-21, so every reported value is now a
% final computed result. \prelim previously flagged partial-coverage numbers;
% it is kept as a transparent pass-through so the macro call sites did not all
% have to be unwound, and so the draft-mode markup remains available if a
% future revision ever reports partial data again.
\makeatletter
\long\def\prelim#1{#1}
\makeatother
...
and SimpleQA. Measured with the usual correlation coefficient, even $N=3$ ensembles behave
like barely more than a single independent voter ($N_\text{eff}=1.08$ for
prompt diversity, $1.43$ for model-family diversity, $1.37$ for all 12
configurations together). We then show that this coefficient flatters such
ensembles, because it is bounded above by a function of the two members'
error rates: 10 of our 12 within-axis pairs sit exactly at that bound.
Looking at the error sets directly explains why. In 44 of 66 configuration
pairs, one member's errors are a strict \emph{subset} of the other's, so the
weaker member recovers nothing the stronger one missed and a majority vote
has no material to work with. That nesting holds for 27 of the 28 pairs drawn
from a single base model and falls to about half for pairs spanning different
labs, against a permutation baseline below 1\% ($p<0.0005$). The consequence
is measurable at the level practitioners care about: majority voting fails to
beat the single best member of its own ensemble on any axis, trailing it by
up to 20 points. We
...
credited with independence they do not have. 10 of our 12 within-axis pairs
sit exactly at that bound.
\item We identify the structure responsible: errors are largely
\emph{nested} rather than overlapping. In 44 of 66 pairs one member's errors
are a strict subset of the other's, including 27 of the 28 same-base-model
pairs, against a permutation baseline below 1\% ($p<0.0005$). A nested pair
has zero complementary errors, so no aggregation rule over it can recover a
question.
\item We show the nesting rate is far higher within a single base model than
across models (96\% against 41\% and 48\% for pairs sharing a lab and spanning
labs; trend $p=1.7\times10^{-4}$), which gives the practitioner guidance a
correlation ranking only gestures at.
\item We connect the structure to the outcome it predicts. Majority voting
never beats the best member of its own ensemble on any axis, trailing it by
up to 20 points, and outside family-diverse only 1 to 2 questions per
ensemble were recoverable by a better aggregation rule.
\item We compare five allocation strategies at equal API cost under all three
measures, and show that the ranking a correlation-based $N_\text{eff}$ would
suggest does not survive the correction (a 32\% apparent advantage for
cross-family ensembling becomes about 2\%). We release the harness, the graded
error matrix, and a decision log recording every deviation from the frozen
specification.
\end{itemize}
...
All results are computed on the complete $30\times12$ design: all 360
(question, configuration) cells were generated and graded, at 75.0\% overall
accuracy. Every pairwise comparison below therefore rests on the same 30
questions, with no missing cells and no coverage confound. Table~\ref{tab:neff}
reports $N_\text{eff}$ per axis with bootstrap 95\% confidence intervals.
Prompt-diverse and temperature-diverse, the two axes that vary only a
surface property of one fixed base model, show the highest mean pairwise
error correlation ($\bar\rho=0.88$ and $0.87$) and collapse to
$N_\text{eff}\approx 1.1$ regardless of their nominal size $N=3$.
Size-diverse is somewhat better ($\bar\rho=0.71$, $N_\text{eff}=1.24$),
consistent with \texttt{config/models.yaml}'s documented caveat that its
``flagship'' tier is a newer Flash-generation model rather than a genuinely
larger one, so this axis likely understates what true parameter-scale
diversity would buy. Family-diverse scores best: mean error correlation
drops to $\bar\rho=0.55$ and $N_\text{eff}$ rises to 1.43, still far from the
nominal $N=3$. Extending this to the full 12-configuration ensemble,
$N_\text{eff}$ reaches only 1.37: adding nine further configurations beyond
$N=3$ buys roughly a third of one additional effective voter.

\begin{table}[!htb]
\centering
\caption{$N_\text{eff}$ per axis under $\phi$ (bootstrap 95\% CI) and under the marginal-robust $\bar{Q}$, on the complete 360-cell design. Every pair is measured on all 30 questions. The $\phi$ columns are inflated by unequal member accuracy (Sec.~\ref{sec:marginalbound}).}
\label{tab:neff}
\begin{tabular}{lccccc}
\toprule
Axis & $\bar\rho$ & $N_\text{eff}(\phi)$ & 95\% CI & $\bar{Q}$ & $N_\text{eff}(Q)$ \\
\midrule
Prompt-diverse & 0.88 & 1.08 & [1.00, 1.27] & 1.00 & 1.00 \\
Temperature-div. & 0.87 & 1.10 & [1.00, 1.29] & 1.00 & 1.00 \\
Size-diverse & 0.71 & 1.24 & [1.07, 1.58] & 0.99 & 1.00 \\
Family-diverse & 0.55 & 1.43 & [1.23, 1.75] & 0.98 & 1.02 \\
\midrule
All 12 configs & 0.71 & 1.37 & [1.18, 1.82] & 0.97 & 1.03 \\
\bottomrule
\end{tabular}
\end{table}

We also measure the confident-correlated-error rate: the fraction of
questions where $\geq$80\% of a $\geq$3-member ensemble is simultaneously
wrong. Across all 12 configurations this occurs on 5 of 30 questions
(16.7\%), and per axis ranges from 13.3\% (size-diverse) to 20.0\%
(prompt-diverse). A majority vote over any of these ensembles returns the
wrong answer confidently on those questions; diversity in surface
presentation does not protect against it. Per-configuration accuracy varies
almost as much within an axis as across axes (individual configurations span
53.3\% to 83.3\%), which matters because an axis can spread its members
widely on accuracy and still have highly correlated errors: $N_\text{eff}$
responds to \emph{which} questions each member gets wrong, not how many.
...
Checking each observed correlation against its own ceiling changes how
Table~\ref{tab:neff} should be read. Ten of the twelve within-axis pairs sit
exactly at their marginal-bound ceiling: their errors are already as
correlated as their differing accuracy permits. This includes two of the
three family-diverse pairs, where one member is markedly more accurate than
the other. For those two, Eq.~\ref{eq:phimax} caps $\phi$ at $0.511$ and
$0.478$, and those are precisely the observed values. Much of the low
correlation on this axis is therefore explained by its members having unequal
accuracy rather than by their failing on complementary questions.

Two of the twelve within-axis pairs fall clearly below their ceiling, the
clearest being the two open-weight models at $\phi=0.665$ against a bound of
$0.935$. Among the within-axis pairs these are the only evidence of error
decorrelation not attributable to an accuracy mismatch.
(Sec.~\ref{sec:nesting} widens the view to all 66 pairs, where cross-lab
pairs supply further such cases.) We therefore do not claim that cross-family
ensembling decorrelates errors. The defensible reading is narrower: measured
correlation is close to the maximum the data permits for most pairs, and much
of what separates the axes in Table~\ref{tab:neff} is how unequal their
...
The result is stark. In 44 of 66 configuration pairs the errors are
fully nested (bootstrap 95\% CI over questions $[67\%, 95\%]$).
Table~\ref{tab:nesting} breaks this down by how genuinely different the two
members are: \textbf{27 of the 28 pairs drawn from the same base model are
nested}, 96\% (Wilson 95\% CI $[82\%, 99\%]$), against 41\% for pairs sharing
a lab but not a model ($[22\%, 64\%]$) and 48\% for pairs spanning different
labs ($[28\%, 68\%]$). A Cochran--Armitage test across the three ordered
groups confirms a decreasing trend ($z=-3.77$, $p=1.7\times10^{-4}$), though
the decrease is not strictly monotonic: the two cross-model categories are
statistically indistinguishable from one another, and the separation that
drives the trend is between same-model pairs and everything else.
Fig.~\ref{fig:nesting} plots the rates.
...
pattern exactly. Over 2{,}000 permutations the expected nesting rate is under
$1\%$ in both cases (95\% range $[0\%, 2\%]$ across all pairs and
$[0\%, 0\%]$ among same-model pairs). Against observed rates of $67\%$ and
$96\%$, no permutation in either null came close to the observed value
($p<0.0005$). At full coverage the chance baseline is far lower than it was
under partial coverage, because empty off-diagonal cells stop being easy to
produce by luck once every pair is measured on all 30 questions. The nesting
is a property of the models, not of the sample size.

A second control addresses the grading circularity of
Sec.~\ref{sec:grading}. Excluding SimpleQA entirely leaves 19 questions,
where nesting rises to $91\%$ (60 of 66 pairs) and same-model pairs remain at
27 of 28. The effect is not produced by the self-judged dataset; if anything
it is stronger without it.

The first row is the paper's sharpest result. Across 27 of the 28 pairs
sharing a base model, there is not one question the second configuration
gets right that the first gets wrong. Changing persona or temperature
reorders and rewords the output, and changes how often the model is wrong,
but in all but one pair it never contributes an independent success. That is
a stronger and more directly checkable claim than $\bar\rho=0.88$, since it
is a count over the complete design rather than an estimated coefficient.
...
off-diagonal cell is empty, so with as few as five errors in some pairs a
...
\begin{table}[!htb]
\centering
\caption{Error nesting on the complete 360-cell design. A pair is nested when one member's errors are a subset of the other's, so the pair has no complementary errors for a vote to exploit.}
\label{tab:nesting}
\begin{tabular}{lcc}
\toprule
Pair type & Nested pairs & Rate \\
\midrule
Same exact model & 27 / 28 & 96\% \\
Same lab, different model & 7 / 17 & 41\% \\
Different lab & 10 / 21 & 48\% \\
\midrule
All pairs & 44 / 66 & 67\% \\
\bottomrule
\end{tabular}
...
\caption{How often one member's errors are a strict subset of the other's, by how genuinely different the two members are on the complete 360-cell design.}
\label{fig:nesting}
\end{figure}
...
Table~\ref{tab:voting} reports the result, and it is one-sided.
\textbf{Majority voting does not beat the single best member of its own
ensemble on any axis.} It loses by $3.3$ points on prompt-, temperature- and
size-diverse alike, and by $20.0$ points on family-diverse. Over all 12
configurations together it reaches $80.0\%$ against $83.3\%$ for the best
member, a deficit of $3.3$ points at twelve times the API cost.
...
question it missed. The recoverable column shows where the exception lies:
for the three same-base-model axes only 1 or 2 questions were available for a
better aggregation rule to rescue, so their failure is not a deficiency of
majority voting specifically but an absence of complementary errors to
aggregate. Family-diverse is the one ensemble with real material to work
with, 6 recoverable questions, and it is also the one where voting loses by
the widest margin: its members disagree often enough to outvote the strongest
of them.
...
\begin{table}[!htb]
\centering
\caption{Majority vote against the members it aggregates, on the complete 360-cell design. ``Recov.'' counts questions where the majority was wrong but at least one member was right. A non-positive vote${-}$best column means voting never beat the strongest member.}
\label{tab:voting}
\begin{tabular}{lccccc}
\toprule
Ensemble & $n_q$ & Vote & Best & Mean & Recov. \\
\midrule
Prompt-diverse & 30 & 76.7 & 80.0 & 76.7 & 1 \\
Temperature-diverse & 30 & 80.0 & 83.3 & 80.0 & 1 \\
Size-diverse & 30 & 80.0 & 83.3 & 78.9 & 2 \\
Family-diverse & 30 & 63.3 & 83.3 & 64.4 & 6 \\
\midrule
All 12 configs & 30 & 80.0 & 83.3 & 75.0 & 2 \\
\bottomrule
\end{tabular}
\end{table}
...
Read through $\phi$, cross-family looks decisively best: $N_\text{eff}=1.43$
against $1.08$ for three prompts of one model, a 32\% gain at identical cost.
Almost all of that gap is the accuracy artifact. Under $Q$ the same
comparison is $1.02$ against $1.00$. We therefore do not claim a 32\%
reliability gain from crossing model families, and a practitioner should not
budget for one.

What survives the correction is visible in the last column. The two
allocations drawn from a single base model are fully nested, 3 pairs of 3:
no member ever answers a question its partners miss. Every allocation that
spans more than one model contains at least one pair with complementary
errors. That is the real distinction at a fixed budget, and it is a
difference in kind rather than a 32\% improvement in a statistic.

This also corrects a reading $\phi$ invites. Under $\phi$ the mixed
allocation ($1.18$) looks much worse than cross-family ($1.43$), suggesting
that spreading a budget across axes poorly substitutes for changing model
family. Under $Q$ the two are close ($1.01$ and $1.02$). What matters is
whether the allocation spans more than one base model at all, rather than how
many \emph{kinds} of diversity it mixes. With differences this small at
$N=30$, we do not rank the non-nested allocations against each other.

\begin{table}[!htb]
\centering
\caption{Allocations at a matched budget of $k=3$ API calls per question, on the complete 360-cell design. Every row costs the same. The $\phi$ column overstates the spread because it is capped by differences in member accuracy (Sec.~\ref{sec:marginalbound}); the $Q$ and nesting columns are the ones to read.}
\label{tab:budget}
\begin{tabular}{lccc}
\toprule
Allocation ($k=3$) & $N_\text{eff}(\phi)$ & $N_\text{eff}(Q)$ & Nested \\
\midrule
Same model, 3 prompts & 1.08 & 1.00 & 3/3 \\
Same model, 3 temperatures & 1.10 & 1.00 & 3/3 \\
Same family, 3 sizes & 1.24 & 1.00 & \textbf{2/3} \\
\midrule
Mixed, one per axis & 1.18 & 1.01 & \textbf{1/3} \\
3 different families & 1.43 & 1.02 & \textbf{2/3} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Dataset size.} With coverage complete, the binding limitation is the
question set: $N=30$ questions ($\sim$8--11 per dataset) leaves several 95\%
intervals wide (size-diverse: $[1.07,1.58]$). This is a power limitation
rather than an artifact of the method, so the axis \emph{ordering} should be
read as descriptive of this dataset rather than as a population ordering. A
larger question set is the single most valuable extension.
\textbf{Both association measures are constrained.} $\phi$ cannot exceed the
Eq.~\ref{eq:phimax} ceiling and 10 of 12 within-axis pairs sit
...
as few as five errors in some pairs one flipped answer would move it. The
...
nesting result intact (91\% overall, same-model pairs still 27 of 28), so we
do not believe it drives the finding, but an independent
grader would be preferable and was unavailable within the quota.

\section{Conclusion}
On the complete design, prompt- and temperature-diversity are the cheapest
and most commonly used axes, and the two that buy the least real
independence: $N_\text{eff}\approx 1.1$ at nominal $N=3$. Model-family
diversity is the axis on which measured correlation falls furthest, but we do
not claim the gain its $\phi$-based $N_\text{eff}$ of $1.43$ implies: that
margin rests largely on pairs whose low correlation is already explained by
unequal member accuracy (Sec.~\ref{sec:marginalbound}). Corrected, the
comparison is $1.02$ against $1.00$.

The ranking is the least robust part of these results. The sturdiest finding
is structural and needs no coefficient: in 44 of 66 pairs one member's errors
are a strict subset of the other's, including 27 of the 28 pairs drawn from
...
of its own ensemble on any axis, and outside family-diverse only 1 to 2
questions per ensemble were even available for a smarter aggregator to
rescue.
...
These remain small-scale results at $N=30$ questions
(Sec.~\ref{sec:limitations}), consistent in direction with classical ensemble
theory and with concurrent independent
work~\cite{kim2026diversitymetrics,ding2026agreement}. The design is now
complete, so the numbers no longer move with coverage, but a larger question
set would be needed before treating the axis ordering as settled.
